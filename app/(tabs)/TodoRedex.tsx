...
import { addToDo, deleteToDo, setToDo, toggleToDoComplete, updateToDo } from "../Redux/todo";
        ...
        const fetchTodos = async () => {
            try {
                ...
                if (user) {
                    ...
                    dispatch(setToDo([])); // If no user, set todos to empty array
                }
            } catch (err) {
    ...
    const addTodo = async (values) => {
        ...
        try {
            ...
            if (user) {
                ...
                dispatch(addToDo({...values, id: data._documentPath._parts[1]}))
                // console.log(todos);
    ...
    const editTodo = async (values) => {
        ...
        try {
            if (todoToEdit) {
                ...
                dispatch(updateToDo({id: todoToEdit.id, value: values}))
            }
        } catch (err) {
...
export default function ToDoRedex(){
    return (
        <Provider store={todoStore} >
            <ToDoComponent />
        </Provider>
